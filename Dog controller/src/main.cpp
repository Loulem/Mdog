...
int buttonPin = 47;             // Bouton-poussoir en broche 7
...
void loop()
{
  ...
  */
  ...
  {
    x = (analogRead(axeX) - (float)(512)) / 512.0;
    y = -(analogRead(axeY) - (float)(512)) / 512.0;
  }
  nrf24_sendData(String(i1) + "," + String(x) + "," + String(y));
}

void nrf24_init(uint8_t channel)
{ // fonction pour iniatialiser le module de communication NRF24
  printf_begin();
  radioNRF.begin();
...
bool nrf24_sendData(String dataToSend)
{ // fonction pour envoyer les données par transimission du NRF24
  // delay(50);
...
{                          // initialise le joystick
  ...
  pinMode(buttonPin, INPUT);     // définition de 7 comme une entrée
  digitalWrite(buttonPin, HIGH); // Activation de la résistance de Pull-Up
}

int get_mouvement()
{ // renvoie 0 si pas de mouvement, 1 pour avancer, -1 pour reculer, 2 pour la droite -2 pour la gauche
...
void printlcd(const char ligne1[], const char ligne2[], const char ligne3[], const char ligne4[])
{ // attention au type de données (possibilité d’améliorer le code en prenant d’autres types à condition de bien déclarer mouvement_print avec les types associés), la fonction peut ne pas fonctionner avec des String!!!
